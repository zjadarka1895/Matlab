function [A,b] = generate_matrix(N)
    ...
    if(convergence_factor<0 || convergence_factor>9)
        error('Wartość convergence_factor powinna należeć do przedziału [1,9]');
    end
    ...
    diag_values = (sum(abs(A),2) - abs(diag(A))) * convergence_factor_2;
    A(1:(N+1):end) = diag_values; % nowe wartości na głównej diagonalnej
