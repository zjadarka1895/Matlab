function [ft_5, integral_1000, Nt, integration_error] = trapezoidal_rule_accuracy_evaluation()
    ...
    for i = 1:length(Nt)
        ...
        integration_result = trapezoidal_rule(x_local);
...
function integral_approximation = trapezoidal_rule(x)
    ...
    delta_x = x(2) - x(1);
    integral_approximation = (failure_density_function(x(1)) + 2 * sum(failure_density_function(x(2:end-1))) + failure_density_function(x(end))) * (delta_x / 2);
end
