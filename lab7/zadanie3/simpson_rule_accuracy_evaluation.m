function [ft_5, integral_1000, Nt, integration_error] = simpson_rule_accuracy_evaluation()
    ...
    for i = 1:length(Nt)
        ...
        integration_result = simpson_rule(x_local);
...
function integral_approximation = simpson_rule(x)
    ...
    integral_approximation = (sum(failure_density_function(x(1:end-1)))+4*sum(failure_density_function(midpoints))+sum(failure_density_function(x(2:end))))* (delta_x / 6);
end
